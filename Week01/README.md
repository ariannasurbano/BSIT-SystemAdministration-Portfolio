# Week 1 – Building My Professional Environment

## Student Information

- **Name:** Arianna Surbano
- **Course:** Bachelor of Science in Information Technology (BSIT)
- **Section:** BSIT 4C
- **Date:** August 7, 2026

---

# Objectives

- Learn how to use GitHub professionally.
- Create a professional portfolio repository.
- Set up GitHub and LinkedIn accounts.
- Learn the basics of version control using Git.
- Develop skills in documentation using Markdown.

---

# Software Installed

- Git
- GitHub
- GitHub Desktop
- Visual Studio Code
- Google Chrome

---

# Professional Accounts

**GitHub:** https://github.com/ariannasurbano

**LinkedIn:** https://www.linkedin.com/in/surbano-arianna-c-7b2546427?utm_source=share_via&utm_content=profile&utm_medium=member_android

---

